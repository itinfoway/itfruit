...
class Cron_jobs extends Controller {
    ...
    public function index() {
        if (date("N", strtotime("+1 day")) != 6 && date("N", strtotime("+1 day")) != 7) {
            ...
            $data = $this->subscription_model->view(["from_date <=" => date("Y-m-d", strtotime("+1 day")), "to_date >=" => date("Y-m-d", strtotime("+1 day"))]);
            foreach ($data as $d) {
                if (is_null($d->last_order_date) || strtotime($d->last_order_date) < strtotime(date("Y-m-d", strtotime("+1 day")))) {
                    ...
                    if ($chargeJson['amount_refunded'] == 0 && empty($chargeJson['failure_code']) && $chargeJson['paid'] == 1 && $chargeJson['captured'] == 1) {
                        ...
                        $order_details["delivered_on_day"] = date("N", strtotime("+1 day"));
                        ...
                        if ($oId) {
                            ...
                            $orderID = "SUBSCRIPTION" .date("Ymd") . str_pad($oId, 4, '0', STR_PAD_LEFT);
